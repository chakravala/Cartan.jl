...
interval_scale(t::FiberBundle) = interval_scale(points(t))
interval_scale(t::ProductSpace{V}) where V = Chain{V}(interval_scale.(split(t)))
function interval_scale(t::AbstractVector)
    x = points(t)
    x[end]-x[1]
end
